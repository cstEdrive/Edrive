...
/* 
 ...
 ec_sim.c: Simulation API layer for Simple Open EtherCAT master
        ...
        M.J.C. Ronde & S.R. Bogemane 24 April 2012: Added EL5002 SSI interface
        M.J.C. Ronde              4 july 2012: First version of simulation library
...
static int nEK1100=0, nEL1008=0, nEL1018=0, nEL1014=0, nEL2004=0, nEL2008=0, nEL3102=0, nEL3104=0, nEL4132=0, nEL4038=0, nEL5002=0, nEL5101=0, nEbox=0;

/* number of ports of specific type */
static int nadc=0, ndac=0, nenc=0, ndin=0, ndout=0, npwm=0;
...
PRIVATE int simulated_experiment(out_Eboxt* pout, in_Eboxt* pin);
...
{
#	define DT	0.00001
	int i,j,id,nsteps,emulation_select;
	double dxsim[4];
	double maxfreq,dt;
    ...
    //printf("Emulation select = %d",emulation_select);
...
//			state equation
			dxsim[0]=xsim[1];
			dxsim[1]=(K1*(pout->aout[0]-xsim[0])+K2*(xsim[2]-xsim[0])+B*(xsim[3]-xsim[1]))/M1;
			dxsim[2]=xsim[3];
...
//		adc: loopback dac
        for (id=0;id<2;id++){
            for (i=0;i<2;i++) {
                (pin+id)->ain[i]=(pout+id)->aout[i];
            }
        }
		#ifdef DEBUG
            printf("loopback: dac0 = %f, adc0 = %f\n", pout->aout[0], pin->ain[0]);
		#endif
        for (id=0;id<2;id++){        
            for (i=0;i<2;i++) {
                (pin+id)->enc[i]=0.0;
...
//		    state equation
	    	dxsim[0]=xsim[1];
		    dxsim[1]=(-PATODAMPINGFW*xsim[1]-PATOSTIFFNESS*(xsim[0]-xsim[2])-PATODAMPING*(xsim[1]-xsim[3])+pout->aout[0])/PATOMASS;
		    dxsim[2]=xsim[3];
...
//			state equation
			dxsim[0]=xsim[1];
			dxsim[1]=(-STIFFNSS*xsim[0]-DAMP*xsim[1]+pout->aout[0])/MASS;
...
//		adc4: acceleration
	...
	case EMULATION_TWOTONE:
		pin->ain[0]= (int32) ((sin(2*PI*325.0*time)+0.001*cos(2*PI*200.0*time))/EBOX_ADC_RESOLUTION);
...
int ec_EL5002_enc_read_chan(double *pvalue, int ichan, int ilink)
{
        int ivalue;

        if ( (ilink<LINK_ID_LOW) || (ilink>LINK_ID_HIGH)|| (ilink>nEL5002-1) ) { return EC_ERR_INVALID_LINK_ID; }
        
        if ( (ichan<0) || (ichan>1) ) { return EC_ERR_INVALID_CHAN_ID; }
        
        switch(ichan){
            case 0:
                ivalue=in_EL5002_[ilink]->channel0;
                break;
            case 1:
                ivalue=in_EL5002_[ilink]->channel0;
                break;
...
{           
        /* Set count0 value to zero, i.e. encoder value = raw encoder value */
        int index_encoder_vars;      
...
int ec_send(void)
{
        /* Add update of simulated system here */
        simulated_experiment(out_Ebox_[0],in_Ebox_[0]);
	...
	for ( cnt=1; cnt<=ec_slavecount ;cnt++) {
        	switch (ec_slave[cnt].eep_id) {
                ...
                case EL1008_DEV_ID: /* digital in */
                        ndin=ndin+EL1008_NCHAN;       
               		break;

                case EL1018_DEV_ID: /* digital in */
                        ndin=ndin+EL1018_NCHAN;
               		break;

                case EL1014_DEV_ID: /* digital in */
                        ndin=ndin+EL1014_NCHAN;
              		break;

                case EL2004_DEV_ID: /* digital out */
                        ndout=ndout+EL2004_NCHAN;
              		break;

                case EL2008_DEV_ID: /* digital out */
                        ndout=ndout+EL2008_NCHAN;
              		break;

                case EL3102_DEV_ID: /* analog in */
                        nadc=nadc+EL3102_NCHAN;
               		break;
                    
                case EL3104_DEV_ID: /* analog in */
                        nadc=nadc+EL3104_NCHAN;
               		break;

                case EL4132_DEV_ID: /* digital out */
                        ndac=ndac+EL4132_NCHAN;
               		break;

                case EL4038_DEV_ID: /* digital out */
                        ndac=ndac+EL4038_NCHAN;
               		break;

                case EL5101_DEV_ID: /* encoder in */
                        nenc=nenc+EL5101_NCHAN;                                
              		break;

                case EBOX_DEV_ID: /* analog in, analog out, encoder in, digital in, digital  out, pwm out */
                case EBOX_DEV_ID2:
                        ndin=ndin+EBOX_DIN_NCHAN;
                        ndout=ndout+EBOX_DOUT_NCHAN;
                        nadc=nadc+EBOX_ADC_NCHAN;
                        ndac=ndac+EBOX_DAC_NCHAN;
                        nenc=nenc+EBOX_ENC_NCHAN;
                        npwm=npwm+EBOX_PWM_NCHAN;
                        npar++;
              		break;
        ...
        if ( nadc>0 ) { printf("   adc:  %d\n", nadc); }
        if ( ndac>0 ) { printf("   dac:  %d\n", ndac); }
        if ( nenc>0 ) { printf("   enc:  %d\n", nenc); }
        if ( ndin>0 ) { printf("   din:  %d\n", ndin); }
        if ( ndout>0 ) { printf("   dout: %d\n", ndout); }
        if ( npwm>0 ) { printf("   pwm:  %d\n", npwm); }
#ifdef DEBUG
...
int ec_start(char* netif)
{
	...
	if (1) {	
		...
		if ( 1 ) {
            ...
            strcpy(ec_slave[1].name,"E/Box");
            ec_slave[1].Obits = 184;
            ec_slave[1].Ibits = 48;
            ec_slave[1].state = EC_STATE_OPERATIONAL;
            ec_slave[1].eep_id = EBOX_DEV_ID;        
            strcpy(ec_slave[2].name,"E/Box");
            ec_slave[2].Obits = 184;
            ec_slave[2].Ibits = 48;
            ec_slave[2].state = EC_STATE_OPERATIONAL;
            ec_slave[2].eep_id = EBOX_DEV_ID;        
...
/*                      allocate memory for ports of specific type */
                        ppadc=(pport_adc) calloc(nadc, sizeof(port_adc));
                        ppdac=(pport_dac) calloc(ndac, sizeof(port_dac));
                        ppenc=(pport_enc) calloc(nenc, sizeof(port_enc));
                        ppdin=(pport_din) calloc(ndin, sizeof(port_din));
                        ppdout=(pport_dout) calloc(ndout, sizeof(port_dout));
                        pppwm=(pport_pwm) calloc(npwm, sizeof(port_pwm));
                        ppebox=(ppar_ebox) calloc(npar, sizeof(par_ebox));
                        
                        
                        /* Create simulation memory */
                        ec_slave[1].outputs  = calloc(1, sizeof(out_Eboxt)) ;
                        ec_slave[1].inputs   = calloc(1, sizeof(in_Eboxt));
                        ec_slave[2].outputs  = calloc(1, sizeof(out_Eboxt)) ;
                        ec_slave[2].inputs   = calloc(1, sizeof(in_Eboxt));
                        
                        if(ec_slave[1].outputs == NULL ||ec_slave[1].inputs == NULL){
				...
				printf("   slave: %d   name: %s   output size: %3d bits   input size: %3d bits   state: %2d\n",
                		...
                		/*slave configuration*/
               			...
               			switch (ec_slave[cnt].eep_id) {
                                ...
                                case EL5002_DEV_ID:
                                    printf("New SSI module found\n");
                                    in_EL5002_[nEL5002] = (in_EL5002t*) ec_slave[cnt].inputs;
                                    nEL5002++;
                                ...
                                case EBOX_DEV_ID2:
                                    ...
                                    if(out_Ebox_[nEbox] == NULL){
                                        printf("Error 1 \n");
                                    } else if(in_Ebox_[nEbox] == NULL){
                                        printf("Error 2 \n");
                                    } else {
                                        printf("Correct \n");
                                    }
