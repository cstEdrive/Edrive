...
g.hgui(1)=figure;
set(g.hgui(1),'Position',[def.XGUI, def.YGUI, def.GUIWIDTH, def.GUIHEIGHT]);
set(g.hgui(1),'MenuBar','none');
set(g.hgui(1),'NumberTitle','Off');
set(g.hgui(1),'Name','E-Scope 1.0');
...
h=def.POPUPHEIGHT;
pos=[x, y, w, h];
g.handles(def.tag_chan_title)=uicontrol_chan_title(pos);

% signal popup
y=y-def.POPUPHEIGHT-def.TITLEHEIGHT-def.YMARGIN/4;
pos=[x, y, w, h];
g.handles(def.tag_chan_signal)=uicontrol_chan_signal(pos);

% enable button
x=x+def.POPUPWIDTH;
w=def.BUTTONHEIGHT;
pos=[x, y, w, h];
g.handles(def.tag_chan_signal_enable)=uicontrol_chan_signal_enable(pos);
...
w=def.SLIDERWIDTH;
pos=[x, y, w, h];
g.handles(def.tag_chan_offset)=uicontrol_chan_offset(pos);

% zeroise button
x=x+def.SLIDERWIDTH;
w=def.BUTTONHEIGHT;
pos=[x, y, w, h];
g.handles(def.tag_chan_offset_zeroise)=uicontrol_chan_offset_zeroise(pos);
...
w=def.POPUPWIDTH;
pos=[x, y, w, h];
g.handles(def.tag_chan_offset_2)=uicontrol_chan_offset_2(pos);

% range popup
y=y-def.POPUPHEIGHT-def.TITLEHEIGHT-def.YMARGIN/4;
pos=[x, y, w, h];
g.handles(def.tag_chan_range)=uicontrol_chan_range(pos);
...
pos=[x, y, w, h];
g.handles(def.tag_meas_title)=uicontrol_meas_title(pos);
...
function h=uicontrol_chan_title(pos)
...
function h=uicontrol_chan_signal(pos)
...
function h=uicontrol_chan_offset(pos)
...
function h=uicontrol_chan_offset_2(pos)
...
function h=uicontrol_chan_range(pos)
...
function h=uicontrol_chan_signal_enable(pos)
...
function h=uicontrol_chan_offset_zeroise(pos)
