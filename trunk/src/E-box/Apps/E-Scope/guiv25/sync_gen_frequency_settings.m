...
% read type
type=tde_gate(def.CALL_FG_GET_TYPE,g.QSG_GEN_SIGNAL-1);

% always do frequency 1
% read gen frequency 1
freq1=tde_gate(def.CALL_FG_GET_FREQ1,g.QSG_GEN_SIGNAL-1);
% set gen frequency 1
tde_gate(def.CALL_FG_SET_FREQ1,freq1,g.QSG_GEN_SIGNAL-1);
...
% bound frequency for fir filter design
    ...
    tde_gate_m(def.CALL_FG_SET_FREQ1,freq1,g.QSG_GEN_SIGNAL-1);
end

if type==def.TWOTONE | type==def.CHIRP | type==def.MULTISINE | type==def.WHISTLE
%   do frequency 2
%   read gen frequency 2
    freq2=tde_gate_m(def.CALL_FG_GET_FREQ2,g.QSG_GEN_SIGNAL-1);
%   set gen frequency 2
    tde_gate_m(def.CALL_FG_SET_FREQ2,freq2,g.QSG_GEN_SIGNAL-1);
