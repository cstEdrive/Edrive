...
//FILE *fp={NULL};
// input buffer index
static int fir_idx_x[MAXFIR];
// input buffer
static double fir_x[FIRNTAPS][MAXFIR];
// fir
static double fir_h[FIRNTAPS][MAXFIR];
...
int fir_design(double rel_stop_freq, int ichan) {
...
double fir_filter(double xn, int ichan)
{
	int i,j;
	double yn;
...
//	store xn
	if (fir_idx_x[ichan]==FIRNTAPS) {
		fir_idx_x[ichan]=0;
	}
	fir_x[fir_idx_x[ichan]][ichan]=xn;
	fir_idx_x[ichan]++;

//	compute filtered output yn
	j=fir_idx_x[ichan]-1;
	for (i=0;i<FIRNTAPS;i++) {
		yn=yn+fir_h[i][ichan]*fir_x[j][ichan];
		j--;
		if (j<0) {j=FIRNTAPS-1;}
	}
