function G=frqls2s(Z,struc,WF,option,WFzinit)
% FRQLS2   G = frqls2s(Z,struc,WF,option,WFzinit)
% output-error-LS stable fit of siso system G to continuous-time and 
...
%           w = vector of frequencies (if negative --> discrete case)
...
% set plotoptions in case plots are required
if plotshow==1,
  hF=figure('name','fqrls');
end;
...
while istop==0,
  ...
  while mreden>tolden&i<iter,
    ...
    %this part always makes sure that the used denominator is a stable one
    %find poles of denominator
    polen=roots(den);
    %find poles equal to zero (on the edge of instability)
    index0=find(polen==0);
    %find unstable poles
    indexr=find(polen>0);
    %flip the real part of the unstable poles in the y-axis
    polen(indexr)=-polen(indexr);
    %make the poles equal to zero somewhat negative
    %polen(index0)=-0.01;
    %convert the new poles to a new denominator
    den=poly(polen);
  ...
  if plotshow==1,
    ...
    hA3=subplot(212);
    bodeplp(wp,[sF sest eF],[0 0 2 0],plstylep);
    hA1=subplot(211);
    bodeplm(wp,[sF sest eF],[0 0],plstyle);
...
%    close(fig)
%    drawnow
    if iplop==0|iplop==4,nlupdate=0;istop=2;end;
...
%      hA=subplot(111);
%      if selz==0,
        bodeplm(wp,[sF sest],[0 0],plstyle);
...
%      end;
      ...
      [y,idx1]=min(abs(wp-x(1)));
      [y,idx2]=min(abs(wp-x(2)));
      plstyleW=[plstylep,'([wp(idx1) wp(idx1)],ax(3:4),''-.'')'];
      eval(plstyleW);
      plstyleW=[plstylep,'([wp(idx2) wp(idx2)],ax(3:4),''-.'')'];
      eval(plstyleW);
      pcpause
