...
%   check for presence of AQI's
    link_id=def.DEVICE_NOT_READY;
    g.QS_AQI=-1*ones(1,def.MAXDEVS);
    for i=1:def.MAXDEVS,
        while link_id==def.DEVICE_NOT_READY
            link_id=tde_gate_m(def.CALL_TD_GET_DEVICE_TYPE,i-1);
            pause(0.1);
        end
        switch link_id
            case 111
                g.QS_AQI(i)=1;
                g.QS_NDEVS=g.QS_NDEVS+1;
                msg=sprintf('Device %d is of type MicroGiant.',i);
                disp(msg);
            case 77
                g.QS_AQI(i)=1;
                g.QS_NDEVS=g.QS_NDEVS+1;
                msg=sprintf('Device %d is of type AQI.',i);
                disp(msg);
            case 74
                g.QS_AQI(i)=0;
                g.QS_NDEVS=g.QS_NDEVS+1;
                msg=sprintf('Device %d is of type QAD.',i);
                disp(msg);
            case 1
                g.QS_NDEVS=g.QS_NDEVS+1;
                msg=sprintf('Device %d is of type E/BOX.',i);
                disp(msg);
            case -1000001
                g.QS_AQI(i)=-1;
                msg=sprintf('Error opening incomm client, E-Scope engine might not be running');
                disp(msg);
            otherwise
                g.QS_AQI(i)=-1;
                %	    msg=sprintf('Device %d is not present.',i);
        end
        %	disp(msg);
        link_id=def.DEVICE_NOT_READY;
    end
...
% check number of TUeDACS devices
if g.QS_NDEVS==0
    error('No E/BOX devices present.');
else
    msg=sprintf('%d E/BOX devices found.',g.QS_NDEVS);
    disp(msg);
end
...
% get max dimensions from TUeDACS Extension library
[g.QS_MAXCHAN g.QS_MAXFRAME]=tde_gate_m(def.CALL_TDE_GET_INFO);
...
% get maxfreq
g.QS_MAXFREQ=tde_gate_m(def.CALL_TDE_GET_MAXFREQ);
...
g.QS_TRIGGER=1;
tde_gate_m(def.CALL_TDE_TRIGGER_ON,g.QS_TRIG_SIGNAL,g.QS_TRIG_TYPE,g.QS_TSCALE(8)*g.QS_TLEVEL(g.QS_TRIG_LEVEL),g.QS_TPREROLL(g.QS_TRIG_PREROLL),g.QS_TRELFREQ(g.QS_TRIG_RELFREQ),g.QS_NSAMP);
...
%   error checking
    ...
    while ~tde_gate_m(def.CALL_TDE_CHECK_ERROR) & g.QS_EXIT==0 & tde_gate_m(def.CALL_TDE_CHECK_DATA) & ~((g.QS_RUNNING==0)&(g.QS_REDRAW==1))
	check_clipping;
        pause(0.1);
    end
    check_clipping;
...
%   if necessary restore QADScope View
    if ~strcmp(get(gcf,'Name'),'QADScope View')
        start_scope;
        g.QS_REDRAW=1;
    end
...
%       set scales and labels for first frame
    ...
    A=tde_gate_m(def.CALL_TDE_GET_DATA,g.QS_NSAMP*g.QS_MAXCHAN*2);
    
%   check which channels are active and which are not
%   stat=zeros(2*g.QS_MAXCHAN,1);
%   stat=tde_gate_m(def.CALL_TDE_GET_ADC_ENC_STATE);
...
%				find cross point openloop with radius 1
    ...
    tde_gate_m(def.CALL_TDE_SAY_READY);
...
[g.QS_CLIPPED, retval]=tde_gate_m(def.CALL_TDE_CHECK_CLIPPING);
