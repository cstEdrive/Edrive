function [Pu,hPu,Qu,hQu]=hzshifpc(Pu,hPu,Qu,hQu)
% WMSHIFT [Pu,hPu,Qu,hQu]=hzshifpc(Pu,hPu,Qu,hQu)
%  shift frequency pulses and intervals horizontally
...
while more
  ...
  if type=='o'
    if butsh==1
      w_shift=xsh-xpick;
      x3=x3pick+w_shift;
      Quk(1:5)=[y3/[ones(1,3); x3 ; x3.^2] x3([1 3])];
      Quk=ffreplot(gca,Quk,hQu(iq(ivq)));
      set(hoq(ivq),'XData',xsh);
    else
      more=0;
      Qu(iq(ivq),1)=Quk(1);
    ...
    if butsh==1
      w_shift=xsh-xpick;
      set(hPu(ip(ivp),1),'XData',xsh*[1 1]);
      set(hop(ivp),'XData',xsh);
    else
      more=0;
