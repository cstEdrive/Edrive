function [num,den]=frqlsbod(sF,w,wp,num,den,nz,nint,option,WF,plstyle);
% [num,den]=frqlsbod(sF,w,wp,num,den,nz,nint,option,WF*WFz,plstyle)
% Nonlinear BODE update of estimate in FRQLS
%                                                           (TUD-WBMR/RS)

% file           : FRQLSBOD.M              ver 1.01
% author         : R. Schrama           (c) TUD-WBMR, 15 feb 1991
% final update   : 4 april 1991
% used functions : MVFREQN, BODEPLM, BODEPLP

disp('Begin NONLINEAR ''Bode'' estimation');
maxiter=option(1)*4;
plotshow=option(2);
tolJ=option(3);
rho=0.78;
...
if plotshow==1,
  ...
  while floor(nltimes)~=nltimes,
   nltimes=input(['How many nonlinear updates? --> ']); 
  end;
...
calM   = 'M=log(sF)-log(P);';
calJ   =['eval(calP);eval(calM);Je=sum(abs(M.*WF).^2);'];
...
istop=0;
while istop==0,
  conv=0;iter=0;Jprev=2*Je;inf=0;
  while iter<nonliter&conv==0, %&inf<3
    iter=iter+1;inf=0;
    % calculate gradients first without C.C.
    eval(calP);eval(calM);kB=1;kA=1;
    for k=N:-1:0,
      Zk=mvfreqn([zeros(1,N-k),1,zeros(1,k)],...
                 [zeros(1,N-nden),1,thetaA',zeros(1,nint)],w);
      if k<=nnum,
        if k>=nz,
          gradB(kB) = -2*sum(real(conj(M).*Zk.*WF.*WF./P)); 
          grad2B(kB)= 2*sum([abs(Zk./P).^2+...
                             real(conj(M).*Zk.*Zk./(P.*P))].*WF.*WF);
          kB=kB+1;
        end;
      end;
      if k<nden,
        if k>=nint,
          gradA(kA)=2*sum(real(conj(M).*Zk.*WF.*WF)); 
          grad2A(kA)= 2*sum([abs(Zk).^2-real(conj(M).*Zk.*Zk)].*WF.*WF);
          kA=kA+1;
        end;
      end;
    end;
    ...
    if floor((iter+2)/3)==(iter+2)/3,gradB=0*gradB;end; % no update of B
    if floor((iter+1)/3)==(iter+1)/3,gradA=0*gradA;end; % no update of A
    ...
    if Je>Jold,  % no update over grad2i>0
      Je=Jold;thetaA=thetaAold;thetaB=thetaBold;inf=1;
      ...
      if size(ixgxxB)~=[0,0], %linstep over B may be profitable
        ...
        if Je>Jold, % stop via no_decrease_of_Je
          ...
          if niter==0, % no linstep update of B
            thetaB=thetaBold;inf=inf+1;
          else,
            thetaB=thetaBold-step*gradB*(rho^(-niter));
          end;
        end;
      else, % no update over B linstep
        inf=inf+1;
      end;
    else, % no grad2B<=0
      inf=inf+1;
    ...
    if inf==3,
      disp(['no further improvement possible at nonl.step ',int2str(iter)]);
    else,
      disp(['nonlin.iter ',int2str(iter),'; Je = ',sprintf('%.8g',Je)]);
    end;
    if floor(iter/3)==iter/3,
      Jrel=(Jprev-Je)/Je;
      if Jrel<tolJ,conv=1;end;
      Jprev=Je;
    end;
    if plotshow==1,
      if iter==nltimes,conv=2;end;
    end;
  end;
  if conv==1,disp(['relative convergence within ',num2str(tolJ)]);end;
  if conv==2,disp([int2str(nltimes),' non-linear iterations performed']);end;
  eval(calP);
